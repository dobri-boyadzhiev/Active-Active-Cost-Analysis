...
@app.route('/api/charts/savings-heatmap')
def api_savings_heatmap():
    """API: Get savings heatmap data (amount vs percentage)."""
    db = AADatabase(DB_PATH)
    ...
    # Get clusters with savings amount and percentage
    results = db.conn.execute('''
        SELECT
            cr.mc_uid,
            cr.total_savings,
    ...
    # Format: [{x: savings_amount, y: savings_percent, v: cluster_count, label: mc_uid}]
    heatmap_data = []

    for row in results:
        heatmap_data.append({
            'x': round(row['total_savings'], 2),
            'y': round(row['savings_percent'], 2),
            'label': row['mc_uid'][:12] + '...'
        })
...
# ============================================================================
# NEW CHART APIs - ACTION PRIORITY
# ============================================================================

@app.route('/api/charts/roi-timeline')
def api_roi_timeline():
    """API: Get ROI timeline projection (waterfall chart data)."""
    db = AADatabase(DB_PATH)
    ...
    # Get top clusters by savings (priority order)
    # 🔴 FIXED: Use .format() instead of f-string
    query = '''
        SELECT
            cr.mc_uid,
    ...
    for i, row in enumerate(results, 1):
        labels.append(f"#{i}")
        incremental.append(round(row['total_savings'], 2))
        cumulative_sum += row['total_savings']
        cumulative.append(round(cumulative_sum, 2))
...
@app.route('/api/charts/pareto-cost-drivers')
def api_pareto_cost_drivers():
    """API: Get Pareto chart data for top cost drivers."""
    db = AADatabase(DB_PATH)
    ...
    # Get all clusters sorted by cost
    # 🔴 FIXED: Use .format() instead of f-string
    query = '''
        SELECT
            cr.mc_uid,
    ...
    total_cost = sum(row['current_price'] for row in results)
    ...
    for i, row in enumerate(results, 1):
        labels.append(f"#{i}")
        costs.append(round(row['current_price'], 2))
        cumulative_cost += row['current_price']
        cumulative_percent.append(round((cumulative_cost / total_cost) * 100, 2) if total_cost > 0 else 0)
...
# ============================================================================
# NEW CHART APIs - ANOMALY DETECTION
# ============================================================================

@app.route('/api/charts/outlier-detection')
def api_outlier_detection():
    """API: Get outlier detection box plot data."""
    db = AADatabase(DB_PATH)
    ...
    query = '''
        ...
        LEFT JOIN cluster_metadata cm ON cr.mc_uid = cm.mc_uid
        WHERE {}
        ORDER BY cm.cloud_provider, cs.total_price
    '''.format(where_clause)
    results = db.conn.execute(query, tuple(params)).fetchall()
    ...
    for row in results:
        ...
        provider_data[provider].append({
            'value': round(row['current_price'], 2),
            'label': row['mc_uid'][:12] + '...'
        })

    # Calculate box plot statistics for each provider
    box_plot_data = []
    for provider, values in provider_data.items():
        costs = [v['value'] for v in values]
        costs.sort()
        ...
        # Find outliers
        outliers = [v for v in values if v['value'] < lower_fence or v['value'] > upper_fence]
